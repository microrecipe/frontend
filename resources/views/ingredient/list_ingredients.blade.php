            ...
            @if (count($ingredients) > 0)
                <div class="row row-cols-2">
                    @foreach ($ingredients as $key => $ingredient)
                        <div class="col p-3">
                            <div class="card shadow w-100 h-100" id="{{ $key }}">
                                <div class="card-body d-flex flex-column">
                                    <div class="d-flex justify-content-between mb-3">
                                        <h3 class="card-text">{{ $ingredient['name'] }}</h3>
                                        @if (session()->get('is_admin'))
                                            <div class="d-flex flex-row">
                                                <a href="{{ route('ingredients.view.edit_ingredient', ['ingredientId' => $ingredient['id']]) }}"
                                                    class="btn btn-outline-primary me-3 w-100 h-100">
                                            ...
                                            <ul class="list-group">
                                                @foreach ($ingredient['nutritions'] as $key => $nutrition)
                                                    <li class="list-group-item d-flex flex-row justify-content-between align-items-center text-bg-light p-3"
                                                        ...
                                                        <p class="m-0">Amount per gram: {{ $nutrition['per_gram'] }}
                                                        </p>
                                                    </li>
                                                @endforeach
                                            </ul>
